...
public function control_post_page($connection, $get, $session, $data,$server){ // affichage de la page quand la méthode de requête n'est pas post
    ...
    return $VIEWPOST->display_post_page($post_title, $creation_date, $text, $author_id, $author_nickname, $modify_post_button, $delete_post_button, $post_id, $show, $like_status, $comment_field_error,$comments,$display_more, $display_less);
    ...
    public function control_timeline_or_user_posts($connection, $session, $get, $order, $timeline_or_user){
        ...
        $num_shown=($starting_point+14> $POST->get_number_of_posts($connection,$session)) ?$POST->get_number_of_posts($connection,$session):$starting_point+14;
        $posts_array= $POST->get_posts($connection, $session['id'], $order, $timeline_or_user);
    ...
    public function control_post_mod($connection,$get,$session, $data,$server){
        $POST = new Post();
        $USER= new User();
